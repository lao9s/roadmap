<x-app>
    <section>
        @if($text = app(\App\Settings\GeneralSettings::class)->welcome_text)
            <div class="prose max-w-none lg:prose-base prose-headings:!mb-0 prose-p:!mt-4 prose-p:text-lg prose-p:font-medium">{!! $text !!}</div>
        @endif
    </section>
            ...
            <div class="flex items-center space-x-1 md:space-x-4 mt-1">
                ...
                <a href="https://www.facebook.com/groups/getmixpost" target="_blank" rel="nofollow noopener"
                   class="flex items-center text-blue-500 hover:text-blue-600 transition-colors ease-in-out duration-200">
                    ...
                    <div class="hidden md:block">Facebook Private Group</div>
                </a>
            </div>
        </div>
    </div>

    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-5 mt-10">
